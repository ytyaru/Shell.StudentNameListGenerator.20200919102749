...
Run() {
	...
	CheckArgs() {
		...
		IsRate "$ARG_RATE" || { Throw '引数 r は r e E のいずれかであるべきです。'; }
	}
	CheckArgs
	...
	random_yomi_mix() {
		local HALF_M=; local HALF_F=; local HALF_C=;
		[ 1 -lt $NUM ] && HALF_M="$(echo -e "$(DB yk om $1)" | shuf -n $(($NUM / 2)))"
		[ 1 -lt $NUM ] && HALF_F="$(echo -e "$(DB yk om $2)" | shuf -n $(($NUM / 2)))"
		[ $(($NUM % 2)) -eq 1 ] && HALF_C="$(echo -e "$(DB yk om c)" | shuf -n 1)"
		echo -e "${HALF_M}\n${HALF_F}\n${HALF_C}" | sed '/^$/d'
	}
	[ '*' = "$ARG_SEX" ] && {
		case "$ARG_RATE" in
			r) RANDOM_YOMI="$(echo -e "$DB" | shuf -n $NUM)";;
			e) RANDOM_YOMI="$(random_yomi_mix m f)";;
			E) RANDOM_YOMI="$(random_yomi_mix M F)";;
		esac
	} || { RANDOM_YOMI="$(echo -e "$DB" | shuf -n $NUM)"; }

	# 各「読み」にあたる「表記」をランダムに1件取得する
	KAKI="$(echo -e "$RANDOM_YOMI" | xargs -I@ sh -c 'echo -e "@" | cut -f2 | tr "," "\n" | shuf -n 1')"

	# 「読み\t表記」のように列結合した結果を出力する
	paste <(echo -e "$RANDOM_YOMI" | cut -f1) <(echo -e "$KAKI")
}
Run "$@"
